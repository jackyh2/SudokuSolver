...
Matrix_ExactCover *m_create_header(std::vector<int> input) {

	Node_Contraint *curr, *prev;
	Matrix_ExactCover *m = new Matrix_ExactCover();

	//First contraint node
	m->head = new Node_Contraint();
	prev = m->head;
	prev->colVal = 0;

	//All other contraint nodes in doubly LL
	for (int i = 1; i < NUMCOLS; ++i) {
		curr = new Node_Contraint();
		curr->colVal = i;
		curr->size = 0;
		curr->left = prev;
		prev->right = curr;
...
Matrix_ExactCover *m_create_cells(Matrix_ExactCover *m, std::vector<int> input) {
